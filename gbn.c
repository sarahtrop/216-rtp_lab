...
#define DATA_LENGTH 20     /* Size of the payload/message */
#define WINDOW_SIZE  8     /* Size of the window for GBN */
#define MAX_BUFFER 50      /* Maximum number of buffers */
...
/*****************************************************************
 ***************** NETWORK EMULTION CODE STARTS BELOW ***********
...
void init() {                       /* initialize the simulator */
  ...
  if (avg < 0.25 || avg > 0.75) {
    ...
    exit(0); /* Added "0" to argument for gcc. JJW 1/14/14 */
  }
...
float jimsrand() {
  ...
  x = (float)drand48();  /* Modified for gcc. JJW 1/14/14 */
  return(x);
}  
...
void generate_next_arrival() {
  double x,log(),ceil();
  struct event *evptr;
  /* char *malloc(); */  /* Removed for gcc. JJW 1/14/14 */
  float ttime;
  int tempint;
